...
public interface IGraphAnalysisService
{
    ...
    Task<GenerateGraphResponse> GenerateRandomGraphAsync(int vertexCount, bool directed, RandomGraphRequestGraph_type graphType = RandomGraphRequestGraph_type.Default, bool includeWeights = false, CancellationToken cancellationToken = default);
    Task<byte[]> GetGraphImageBytesAsync(GraphEntity graph, CancellationToken cancellationToken = default);
    Task<ShortestPathsResponse> GetShortestPathsAsync(GraphEntity graph, string source, CancellationToken cancellationToken = default);
}
